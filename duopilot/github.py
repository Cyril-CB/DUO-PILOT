...
def _get_json(repo, endpoint, params, token):
    return _request_json(repo, endpoint, params, token, expected=list)


def _get_object(repo, endpoint, params, token, max_bytes=1_000_000):
    result, _ = _request_json(repo, endpoint, params, token, expected=dict, max_bytes=max_bytes)
    return result


def _request_json(repo, endpoint, params, token, *, expected, max_bytes=1_000_000):
    if not re.fullmatch(r"[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+", repo):
        raise GitHubError("Dépôt GitHub invalide.")
    if endpoint not in {'issues', 'pulls', 'commits'} and not re.fullmatch(r'git/(trees|blobs)/[0-9a-f]{40}', endpoint):
        raise GitHubError("Lecture GitHub non autorisée.")
    headers = {
        "Accept": "application/vnd.github+json",
        "X-GitHub-Api-Version": "2022-11-28",
        "User-Agent": "Duo-Pilot-experiment/0.3",
    }
    if token:
    ...
    try:
        ...
        with requests.get(
            ...
            if not isinstance(result, expected) or (expected is list and not all(isinstance(item, dict) for item in result)):
...
def build_context(config: Mapping, topic: str, mode: str) -> str:
    ...
    evidence = {
        "repository": repo,
        "base_branch": branch,
        "snapshot_commit": _clip(commits[0].get('sha'), 40) if commits else None,
        "snapshot_tree": ((commits[0].get('commit') or {}).get('tree') or {}).get('sha') if commits else None,
        "limitations": (
            "Instantané partiel : 20 entrées issues au maximum (PR exclues ensuite), "
            "4 PR ouvertes, 5 commits. Corps et titres abrégés. "
            "Ces résumés ne lisent pas les sources, diffs, commentaires, CI ou reviews. "
            "Seuls les extraits fournis séparément attestent une lecture de fichier ; "
            "une issue ou un commit ne confirme pas un bug."
        ),
